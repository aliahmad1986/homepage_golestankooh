  ...
  private loadCalenderData() {
    ...
    if (this.RentItemID) {
      ...
      this.libReservPanelService
        ...
        .subscribe(
          (data) => {
            this.cellCalenderReservations = data.result.data.data as Month[];
            this.starttDate = data.result.data.startDate as string;
            this.next = data.result.data.next as boolean;
            this.prev = data.result.data.prev as boolean;
          },
          error => { },
        );
    } else {
