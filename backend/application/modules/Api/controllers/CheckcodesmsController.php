...
class Api_CheckcodesmsController extends REST_Controller
{
    public function postAction()
    {
        try {
            ...
            if ($post['api_key'] == $api_key) {
                $code = $post['code'];
                $hashdata = $post['hashdata'];
                $mobile = $modelApi->GetValue($post['registeredMobile'], 'موبایل', true);
                $data=$modelApi->checkcodeService($code,$hashdata,$mobile);
